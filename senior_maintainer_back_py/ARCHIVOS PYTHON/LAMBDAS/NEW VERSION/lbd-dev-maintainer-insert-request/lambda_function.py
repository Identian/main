...
def execute_query(entry_query):
    try:
        ...
        if sql_connection.is_connected():
            ...
            try:
                logger.info(f"[execute_query] query a ejecutar para ingresar la solicitud: \n{entry_query}")
                sql_cursor.execute(entry_query)
                logger.info(f"[execute_query] query ejecutada correctamente")
            except mysql.connector.Error as err:
                logger.error(f"[execute_query] Error ejecutando el query, motivo: {str(err)}")
                exception_line = sys.exc_info()[2].tb_lineno
                current_error = err
        ...
        global request_is_inserted
        request_is_inserted=True
        response_msg = "Se agregó la solicitud a la base de datos pero no se envió."
        logger.info(f"[execute_query] Termina este else y request_is_inserted debería se true; es {request_is_inserted}")
    finally:
        sql_cursor.close()
        sql_connection.close()
        logger.info("[execute_query] Termina este finally")
...
def insert_transaction(data_entry):
    try:
        ...
        logger.info(f"[insert_transaction] request_is_inserted debería se true; es {request_is_inserted}")
        if request_is_inserted:
            logger.info(f"[insert_transaction] Esta entrando al if de enviar mensaje de transaccion")
            send_transaction_message(data_entry)
    except Exception as lambda_handler_error:
        logger.info(f"[insert_transaction] Esta entrando a la excepcion, algo falló, motivo: {lambda_handler_error}")
        exception_line = sys.exc_info()[2].tb_lineno
